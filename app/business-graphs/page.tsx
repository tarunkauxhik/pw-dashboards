...
import { BusinessGraphsDashboard } from "@/components/BusinessGraphsDashboard";

export const dynamic = "force-dynamic";

export default async function BusinessGraphsPage() {
  const result = await fetchSheetCached();
  if (!result.ok || !result.data) {
    return (
      <DashboardShell
        title="Business Graphs"
        subtitle="Trend graphs (MoM, WoW, Last 7 Days) over headline KPIs."
        sidebarMeta={{
    ...
    >
      {data.mb_orders.length === 0 ? (
        <div className="rounded-lg border border-dashed border-border/60 bg-card/40 p-6 text-sm text-muted-foreground">
          ...
          <p className="mt-1">
            The Sheet API returned empty rows. Hit the refresh button in the
            sidebar to retry.
          </p>
        </div>
      ) : (
        <BusinessGraphsDashboard data={data} anchor={anchorDate} />
      )}
    </DashboardShell>
  );
}
